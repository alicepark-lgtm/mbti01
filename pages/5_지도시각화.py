...
COORDINATES = {
    # 상록구 (기본 테마: 파란색 계열)
    "일동": {"구": "상록구", "위도": 37.3075, "경度": 126.8647},
...
def load_and_prepare_data():
    ...
    if pop_df is not None:
        # 💡 전처리 파일의 컬럼명 규격인 'Year', 'Dong', 'Gu' 구조에 맞춰 필터링합니다.
        # 만약 전처리 파일 내부 컬럼명이 한글('연도', '동')인 경우를 모두 대응할 수 있도록 처리합니다.
...
if df is not None and not df.empty:
    ...
    for _, row in df.iterrows():
        ...
        # 💡 매핑된 한글 컬럼명인 '총인구수', '남자인구수', '여자인구수'를 안전하게 호출합니다.
        popup_html = f"""
        <div style="
